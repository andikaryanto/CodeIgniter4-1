<?php  
namespace App\Entities;

use App\Classes\Exception\EntityException;
use App\Entities\BaseEntity;
use App\Libraries\ResponseCode;

class M_VillageEntity extends BaseEntity {

	public $Id;
	public $M_Subdistrict_Id;
	public $Name;
	public $Description;
	public $Headman;
	public $Phone;
	public $Latitude;
	public $Longitude;
	public $CreatedBy;
	public $ModifiedBy;
	public $Created;
	public $Modified;

    
    protected $primaryKey = "Id";

    public function __construct(){
        parent::__construct();
	}
	
	public function validate(M_VillageEntity $oldmodel = null){

		$nameexist = false;
        $warning = array();

        if(!empty($oldmodel))
        {
            if($this->Name != $oldmodel->Name)
            {
				$params = [
					"Name" => $this->Name,
					"M_Subdistrict_Id" => $this->M_Subdistrict_Id
				];
                $nameexist = $this->isDataExist($params);
            }
        }
        else{
            if(!empty($this->Name))
            {
				$params = [
					"Name" => $this->Name,
					"M_Subdistrict_Id" => $this->M_Subdistrict_Id
				];
                $nameexist = $this->isDataExist($params);
            }
            else{
                throw new EntityException(lang('Error.name_can_not_null'), $this, ResponseCode::INVALID_DATA);
            }
        }
        if($nameexist)
        {
            throw new EntityException(lang('Error.name_exist'), $this, ResponseCode::DATA_EXIST);
        }
        
    }
    
    public function calculatedFamilyCard(){

        $result = $this->db->query("
            SELECT a.`M_Village_Id` ,RW, COUNT(RW) KK
            FROM m_familycards a
            INNER JOIN `m_familycardmembers` c ON c.`M_Familycard_Id` = a.Id
            WEHRE a.`M_Village_Id` = {$this->Id}
            GROUP BY a.`M_Village_Id`, RW
        ")->getResult();

        return $result;

    }


}