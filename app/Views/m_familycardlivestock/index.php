...
<section>
    <div class="container-fluid">
        <!-- Page Header-->
        <header>
            <h1 class="h3 display"><?= lang('Form.familycardlivestock') ?> </h1>
            </tr>
        </header>
        <div class="row">
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            <div class="col-6">
                                <h4><?= lang('Form.livestock') . " " . $model->FamilyCardNo?></h4>
                            </div>
                            <div class="col-6 text-right">
                                <a class = "link-action" href="<?= baseUrl("mfamilycard") ?>"><i class="fa fa-table "></i> <?= lang('Form.familycard') ?></a>
                                <a href="<?= baseUrl("mfamilycardlivestock/add/{$model->Id}") ?>"><i class="fa fa-plus"></i> <?= lang('Form.add') ?></a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
...
<script>
    ...
    function dataTable() {
        var table = $('#tableOwner').DataTable({
            ...
            "serverSide": true,
            ajax: {
                url: "<?= baseUrl("mfamilycardlivestock/getAllData/{$model->Id}") ?>",
                dataSrc: 'data'
            },
            stateSave: true
        });
        ...
        table.on('click', '.delete', function(e) {
            ...
            deleteData(name, function(result) {
                if (result == true) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<?= baseUrl('mfamilycardlivestock/delete/'); ?>",
                        data: {
                            id: data[0]
                        },
