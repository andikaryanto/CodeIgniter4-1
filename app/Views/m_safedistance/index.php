          ...
          <div class="card-header">
            <div class = "row">
              ...
              <div class="col-6 text-right">
                <a href="<?= baseUrl('msafedistance/add')?>"><i class = "fa fa-plus"></i> <?= lang('Form.add')?></a>
              </div>
            </div>
          </div>
          <div class="card-body">
              ...
              <table id = "tableGroupUser" style="width: 100%;" class="table table-striped table-no-bordered table-hover dataTable dtr-inline collapsed " role="grid">
                <thead class=" text-primary">
                  <tr role = "row">
                    ...
                    <th><?=  lang('Form.level')?></th>
                    <th><?=  lang('Form.recommend')?></th>
...
<script>
  ...
  function dataTable(){
    var table = $('#tableGroupUser').DataTable({
      ...
      "serverSide": true,
      ajax:{
        url : "<?= baseUrl('msafedistance/getAllData')?>",
        dataSrc : 'data'
      },
      stateSave: true
    }); 
     ...
     table.on( 'click', '.delete', function (e) {
        ...
        deleteData(name, function(result){
          if (result==true)
          {
            ...
            $.ajax({
              type : "POST",
              url : "<?= baseUrl('msafedistance/delete/');?>",
              data : {id : data[0]},
