...
<section>
    <div class="container-fluid">
        <!-- Page Header-->
        <header>
            <h1 class="h3 display"><?= lang('Form.familycardmember') ?> </h1>
            </tr>
        </header>
        <div class="row">
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            <div class="col-6">
                                <h4><?= lang('Form.member') . " " . $model->FamilyCardNo?></h4>
                            </div>
                            <div class="col-6 text-right">
                                <a class = "link-action" href="<?= baseUrl("mfamilycard") ?>"><i class="fa fa-table "></i> <?= lang('Form.familycard') ?></a>
                                <a href="<?= baseUrl("mfamilycardmember/add/{$model->Id}") ?>"><i class="fa fa-plus"></i> <?= lang('Form.add') ?></a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <table id="tableOwner" style="width: 100%;" class="table table-striped table-no-bordered table-hover dataTable dtr-inline collapsed " role="grid">
                            <thead class=" text-primary">
                                <tr role="row">
                                    ...
                                    <th><?= lang('Form.relation') ?></th>
                                    <th><?= lang('Form.placeofbirth') ?></th>
                                    <th><?= lang('Form.dateofbirth') ?></th>
...
<script>
    ...
    function dataTable() {
        var table = $('#tableOwner').DataTable({
            ...
            "serverSide": true,
            ajax: {
                url: "<?= baseUrl("mfamilycardmember/getAllData/{$model->Id}") ?>",
                dataSrc: 'data'
            },
            stateSave: true
        });
        ...
        table.on('click', '.delete', function(e) {
            ...
            deleteData(name, function(result) {
                if (result == true) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<?= baseUrl('mfamilycardmember/delete/'); ?>",
                        data: {
                            id: data[0]
                        },
