...
<section>
  <div class="container-fluid">
    <!-- Page Header-->
    <header> 
          <h1 class="h3 display"><?= lang('Form.disastervictim')." ({$model->TransNo} ~ {$model->get_M_Disaster()->Name})"?> </h1>
      </tr>
    </header>
    <div class="row">
          ...
          <div class="card-header">
            <div class = "row">
              ...
              <div class="col-6 text-right">
                ...
                <a href="<?= baseUrl("tdisasteroccurvictim/add/{$model->Id}")?>"><i class = "fa fa-plus"></i> <?= lang('Form.add')?></a>
              </div>
            </div>
          </div>
          <div class="card-body">
            <div class="table-responsive">
              <table id = "tableGroupUser" style="width: 100%;" class="table table-striped table-no-bordered table-hover dataTable dtr-inline collapsed " role="grid">
                <thead class=" text-primary">
                  <tr role = "row">
                    <th># </th>
                    <th><?=  lang('Form.disastervictim')?></th>
                    <th>NIK</th>
...
<script>
  ...
  function dataTable(){
    var table = $('#tableGroupUser').DataTable({
      ...
      "serverSide": true,
      ajax:{
        url : "<?= baseUrl("tdisasteroccurvictim/getAllData/{$model->Id}")?>",
        dataSrc : 'data'
      },
      stateSave: true
    }); 
     ...
     table.on( 'click', '.delete', function (e) {
        ...
        deleteData(name, function(result){
          if (result==true)
          {
            ...
            $.ajax({
              type : "POST",
              url : "<?= baseUrl('tdisasteroccurvictim/delete/');?>",
              data : {id : data[0]},
