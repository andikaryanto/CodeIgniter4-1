...
<section>
  <div class="container-fluid">
    <!-- Page Header-->
    <header>
      <h1 class="h3 display"><?= lang('Form.disasteroccur') ?> </h1>
      </tr>
    </header>
    <div class="row">
          ...
          <div class="card-header">
            <div class="row">
              ...
              <div class="col-6 text-right">
                <a href="<?= baseUrl('tdisasteroccur/add') ?>"><i class="fa fa-plus"></i> <?= lang('Form.add') ?></a>
              </div>
            </div>
          </div>
          <div class="card-body">
...
<script>
  ...
  function dataTable() {
    var table = $('#tableGroupUser').DataTable({
      ...
      "serverSide": true,
      ajax: {
        url: "<?= baseUrl('tdisasteroccur/getAllData') ?>",
        dataSrc: 'data'
      },
      stateSave: true
    });
    ...
    table.on('click', '.delete', function(e) {
      ...
      deleteData(name, function(result) {
        if (result == true) {
          ...
          $.ajax({
            type: "POST",
            url: "<?= baseUrl('tdisasteroccur/delete/'); ?>",
            data: {
              id: data[0]
            },
    ...
    table.on('click', '.picture', function(e) {
      ...
      $.ajax({
        type: "POST",
        url: "<?= baseUrl('tdisasteroccur/getDataById'); ?>",
        data: {
          id: data[0]
        },
        success: function(data) {
