...
<section>
    <div class="container-fluid">
        <!-- Page Header-->
        <header>
            <h1 class="h3 display"><?= lang('Form.masterequipmentowner') ?> </h1>
            </tr>
        </header>
        <div class="row">
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            <div class="col-6">
                                <h4><?= lang('Form.owner') . " " . $model->Name?></h4>
                            </div>
                            <div class="col-6 text-right">
                                <a class = "link-action" href="<?= baseUrl("mequipment") ?>"><i class="fa fa-table "></i> <?= lang('Form.equipment') ?></a>
                                <a href="<?= baseUrl("mequipmentowner/add/{$model->Id}") ?>"><i class="fa fa-plus"></i> <?= lang('Form.add') ?></a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
...
<script>
    ...
    function dataTable() {
        var table = $('#tableOwner').DataTable({
            ...
            "serverSide": true,
            ajax: {
                url: "<?= baseUrl("mequipmentowner/getAllData/{$model->Id}") ?>",
                dataSrc: 'data'
            },
            stateSave: true
        });
        ...
        table.on('click', '.delete', function(e) {
            ...
            deleteData(name, function(result) {
                if (result == true) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<?= baseUrl('mequipmentowner/delete/'); ?>",
                        data: {
                            id: data[0]
                        },
