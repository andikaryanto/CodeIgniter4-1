          ...
          <div class="card-header">
            <div class="row">
              ...
              <div class="col-6 text-right">
                <a href="<?= baseUrl('mfamilycard/add') ?>"><i class="fa fa-plus"></i> <?= lang('Form.add') ?></a>
              </div>
            </div>
          </div>
          <div class="card-body">
...
<script>
  ...
  function dataTable() {
    var table = $('#tableGroupUser').DataTable({
      ...
      "serverSide": true,
      ajax: {
        url: "<?= baseUrl('mfamilycard/getAllData') ?>",
        dataSrc: 'data'
      },
      stateSave: true
    });
    ...
    table.on('click', '.delete', function(e) {
      ...
      deleteData(name, function(result) {
        if (result == true) {
          ...
          $.ajax({
            type: "POST",
            url: "<?= baseUrl('mfamilycard/delete/'); ?>",
            data: {
              id: data[0]
            },
    ...
    table.on('click', '.member', function(e) {

      $tr = $(this).closest('tr');
      var data = table.row($tr).data();
      var id = data[0];
      window.location = "<?= baseUrl('mfamilycardmember/'); ?>" + id;
    });
